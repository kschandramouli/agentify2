...
output "log_platform_glue_database" {
  value       = try(aws_glue_catalog_database.logs[0].name, null)
  description = "Glue database name alone (backend's ATHENA_DATABASE env var — see GET /api/log-source)."
}

output "log_platform_glue_table_name" {
  value       = try(aws_glue_catalog_table.logs[0].name, null)
  description = "Glue table name alone, no database prefix (backend's ATHENA_TABLE env var — see GET /api/log-source)."
}
